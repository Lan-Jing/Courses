...
def getCommonWords(wordDict, trainFile, testFile, ifTest):
    ...
    with open(file="..\\..\\lab1_data\\classification_dataset\\{}".format(trainFile), mode="r") as fp:
        lineCount = -1
    ...
    with open(file="..\\..\\lab1_data\\classification_dataset\\{}".format(testFile), mode="r") as fp:
        lineCount = -1
        for line in fp.readlines():
            # count lines and skip line #1
            lineCount += 1
...
def TFIDF_file(wordDict, fileName, ifTest):
    ...
    with open(file="..\\..\\lab1_data\\classification_dataset\\{}".format(fileName), mode="r") as fp:
        lineCount = -1
        for line in fp.readlines():
            lineCount += 1
            if lineCount == 0:
